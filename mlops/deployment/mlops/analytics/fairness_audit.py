...
def run_fairness_audit(df_predictions, protected_attribute='Geography'):
    ...
    for group, group_df in df.groupby(protected_attribute):
        ...
        if 'Risk_Status' in group_df.columns:
            high_risk = np.sum(group_df['Risk_Status'].astype(str).str.contains('HIGH'))
        elif 'Churn_Probability_%' in group_df.columns:
            high_risk = np.sum(group_df['Churn_Probability_%'] >= 45.0)
        elif 'Churn_Probability' in group_df.columns:
            high_risk = np.sum(group_df['Churn_Probability'] >= 0.45)
        elif 'Age' in group_df.columns:
            high_risk = np.sum(group_df['Age'] >= 45)
        else:
            high_risk = 0
