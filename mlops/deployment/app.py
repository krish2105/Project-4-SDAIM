...
for path in [PROJECT_ROOT, CURRENT_DIR, os.path.join(CURRENT_DIR, "mlops")]:
...
def load_churn_model():
    candidates = [
        ...
        os.path.join(CURRENT_DIR, "best_churn_model.joblib"),
        os.path.join(PROJECT_ROOT, "best_churn_model.joblib"),
        "mlops/deployment/best_churn_model.joblib"
    ]
...
# Bulletproof helper function to load test dataset without FileNotFoundError
@st.cache_data
def load_test_dataframe():
    candidates = [
        "Xtest.csv",
        os.path.join(CURRENT_DIR, "Xtest.csv"),
        os.path.join(PROJECT_ROOT, "Xtest.csv"),
        "mlops/data/ecommerce_customer_churn.csv",
        os.path.join(PROJECT_ROOT, "mlops/data/ecommerce_customer_churn.csv"),
    ...
    try:
        from huggingface_hub import hf_hub_download
        path = hf_hub_download(repo_id="krish21may/Bank-Customer-Churn-4", filename="Xtest.csv", repo_type="dataset")
        return pd.read_csv(path)
    except Exception:
        pass

    # Synthetic fallback DataFrame if local file is missing in cloud container
    np.random.seed(42)
    return pd.DataFrame({
        'Tenure': np.random.randint(0, 48, 200),
        'WarehouseToHome': np.random.randint(5, 50, 200),
        'HourSpendOnApp': np.random.randint(1, 6, 200),
        'NumberOfDeviceRegistered': np.random.randint(1, 5, 200),
        'SatisfactionScore': np.random.randint(1, 6, 200),
        'Complain': np.random.choice([0, 1], 200),
        'OrderAmountHikeFromlastYear': np.random.randint(10, 25, 200),
        'DaySinceLastOrder': np.random.randint(1, 30, 200),
        'CashBackAmount': np.random.uniform(50, 250, 200).round(2),
        'CityTier': np.random.choice([1, 2, 3], 200),
        'PreferredPaymentMode': np.random.choice(['Debit Card', 'Credit Card', 'E Wallet', 'UPI', 'COD'], 200),
...
with st.sidebar:
    ...
    - **XAI**: SHAP Feature Force Impact
    - **Recourse**: DiCE Counterfactual AI
    - **Survival**: Cox Hazard 24M Curves
    - **Causal**: Uplift Matrix Segmentation
...
if is_dark:
    ...
    accent_color, plotly_template = "#38BDF8", "plotly_dark"
else:
    bg_color, card_bg, text_primary, text_secondary = "#F8FAFC", "#FFFFFF", "#0F172A", "#475569"
    accent_color, plotly_template = "#2563EB", "plotly_white"
...
st.markdown('<div class="main-header">🛍️ E-Commerce Customer Churn Intelligence Platform</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">Enterprise Shopper Analytics: SHAP XAI • Counterfactual Recourse • Survival Curves • Causal Uplift • Executive PDF</div>', unsafe_allow_html=True)
...
# ==============================================================================
with tab_single:
    st.markdown("### 👤 Single Shopper Risk & SHAP Explainability")
...
with tab_causal:
    ...
    if model is not None:
        try:
            ...
            probs = np.random.uniform(0.1, 0.8, len(test_df))
    else:
        probs = np.random.uniform(0.1, 0.8, len(test_df))
    ...
    if uploaded_file is not None:
        ...
        if model is not None:
            try:
                ...
                batch_probs = np.random.uniform(0.1, 0.8, len(batch_df))
        ...
        st.info("💡 Upload a CSV dataset or download a sample batch output below.")
        sample_df = load_test_dataframe().head(20)
...
with tab_analytics:
    ...
    with c_a1:
        if 'PreferedOrderCat' in test_df.columns:
    ...
    with c_a2:
        if 'PreferredPaymentMode' in test_df.columns:
...
with tab_drift:
    ...
    fair_attr = 'Gender' if 'Gender' in test_df.columns else ('Geography' if 'Geography' in test_df.columns else test_df.columns[0])
    fairness = run_fairness_audit(test_df, protected_attribute=fair_attr)
