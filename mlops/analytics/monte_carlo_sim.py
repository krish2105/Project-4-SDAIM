...
def run_monte_carlo_simulation(df_predictions, num_simulations=1000):
    ...
    base_probs = df['Churn_Probability'].values if 'Churn_Probability' in df.columns else (df['Churn_Probability_%'].values / 100.0)
    balances = df['Balance'].values if 'Balance' in df.columns else np.full(len(base_probs), 50000.0)
